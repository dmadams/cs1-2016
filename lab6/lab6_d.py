...
class BouncingBall:
    ...
    def displacement(self, c, d, cmax):
        ...
        if (c + self.radius) + d > cmax:
            return d - (d - cmax + c + self.radius)
        elif (c - self.radius) + d < 0:
            return d - (d + c - self.radius)
        return d
...
def random_position(xmin, xmax, ymin, ymax):
    '''Generates a tuple representing a random coordinate of the form (x, y) in
    a 2D coordinate system. 0 <= x <= xmax and 0 <= y <= ymax.
    '''
...
def random_ball(canvas, rmin, rmax, smin, smax, xmax, ymax):
    ...
    center = random_position(radius, xmax - radius, radius, ymax - radius)
    color = random_color()
    direction = 360.0 * random.random()
    speed = random.randint(smin, smax)
    ball = BouncingBall(canvas, center, radius, color, direction, speed)
    return ball
